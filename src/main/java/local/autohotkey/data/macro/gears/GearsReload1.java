...
@Slf4j
public class GearsReload1 implements Macro {
    ...
    public void run() {
        try {
            ...
            log.info("{} {} for x1:{} x2:{} y:{}", color, color1, x1, x2, y);
    ...
    private boolean isWhite(Color color) {
        return color.getBlue() > 246 &&  color.getRed() > 246 && color.getGreen() > 246;
    }

    private int findReloadDelta(BufferedImage img) {
        for(int i = x1 - length; i <= x1; i++){
            log.info("{}",new Color(img.getRGB(i, y)));
            if (isWhite(new Color(img.getRGB(i, y)))) {
                log.info("found at {} {}", i, y);
                return  x1 - i + 1;
            }
        }
        return 0;
